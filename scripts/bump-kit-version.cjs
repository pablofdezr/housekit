...
const pkgPath = path.join(__dirname, '..', 'packages', 'kit', 'package.json');
const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));

// Parse version arguments
const versionArgs = parseVersionArgs();

console.log(`🔧 Bumping @housekit/kit (${formatVersionArgsDisplay(versionArgs)})`);
...
try {
    ...
    registryVersion = execSync(
        'npm view @housekit/kit version --registry https://registry.npmjs.org',
        execOpts
    ).toString().trim() || '0.0.0';
} catch {
...
try {
    ...
    console.log(`✅ Bumped @housekit/kit to ${pkg.version} (remote was ${registryVersion})`);
} catch (error) {
